...
R=${RANDOM}
echo "$((${R}%60)) $((${R}%24)) * * $((${R}%7)) root /uar/local/certbot/certbot-auto renew --webroot -w ${HTTP_DOCROOT} --post-hook 'systemctl reload nginx' --post-hook 'systemctl reload postfix'" > /etc/cron.d/certbot-auto
